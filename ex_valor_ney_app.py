...
def main():
    ...
    selected_clubs = st.sidebar.multiselect(
        ...
        default=df_market_value['Club'].unique().tolist() # Seleciona todos por padrão
    )
    ...
    st.header("Gráfico da Evolução do Valor de Mercado")
    st.markdown("Acompanhe as mudanças no valor de mercado ao longo do tempo para cada clube.")

    # Configurações de visualização com Seaborn
    sns.set_theme(style="whitegrid", palette="viridis") # Você pode experimentar outras paletas como "plasma", "magma", "rocket"

    fig, ax = plt.subplots(figsize=(14, 7))

    # Plota o gráfico de linha com Seaborn
    sns.lineplot(data=df_filtered, x='Date', y='Market_Value_EUR_M', hue='Club', marker='o',
                 linewidth=2.5, ax=ax, markersize=8)

    # Adiciona linhas verticais para indicar as transições de clube
    for club, period in club_periods.items():
        if club in selected_clubs: # Desenha a linha apenas para os clubes selecionados
            ax.axvline(x=period["start"], color=period["color"], linestyle=':', linewidth=1.5,
                       label=f"Início no {club}", zorder=0) # zorder para ficar atrás da linha principal
            # Adiciona anotação da transição
            # Ajuste a posição y das anotações se elas se sobrepuserem
            # Ex: y=ax.get_ylim()[1] * 0.9 para colocar perto do topo
            # Ou, mais complexo, ajustar dinamicamente
            # ax.text(period["start"], ax.get_ylim()[1] * 0.95, f'Início no {club}',
            #         rotation=90, va='top', ha='left', color=period["color"], fontsize=10)


    ax.set_title('Evolução do Valor de Mercado de Neymar (em milhões de €)', fontsize=18, fontweight='bold')
    ax.set_xlabel('Data', fontsize=14)
    ax.set_ylabel('Valor de Mercado (€ Milhões)', fontsize=14)
    ax.legend(title='Clube', fontsize=12, title_fontsize=14, loc='upper left', bbox_to_anchor=(1,1)) # Legenda fora do gráfico
