...
# Define as datas de início em cada clube para as linhas verticais
club_periods_starts = {
    "Santos - 1ª Passagem": "2009-01-01",
    "Barcelona": "2013-07-01",
    "Paris SG": "2017-08-03",
    "Al-Hilal": "2023-08-15",
    "Santos - 2ª Passagem": "2024-12-13"
}
...
# Cores para as linhas verticais de transição
...
def main():
    ...
    # Também filtrar as datas de início para as linhas verticais
    ...
    line_chart = alt.Chart(df_filtered).mark_line(point=True, strokeWidth=2.5).encode(
        ...
        y=alt.Y('Market_Value_EUR_M:Q', title='Valor de Mercado (€ Milhões)', axis=alt.Axis(format='~s')), # Formato mais limpo para milhões
        ...
        tooltip=[ # O que aparece no tooltip
            ...
            alt.Tooltip('Market_Value_EUR_M:Q', title='Valor (€ Milhões)', format='~s'), # 'Q' para quantitativo, '~s' para formato de número significativo
    ...
    # --- Linhas Verticais para Transições de Clube ---
    ...
    vertical_lines = alt.Chart(club_periods_starts_filtered).mark_rule(strokeDash=[3,3], strokeWidth=2).encode(
        x='Start_Date:T', # Data de início
        color=alt.Color('Club:N', scale=transition_line_colors, legend=alt.Legend(title="Início no Clube")), # Cor da linha vertical
        tooltip=[alt.Tooltip('Club:N', title='Início no Clube'), alt.Tooltip('Start_Date:T', title='Data', format='%Y-%m-%d')]
    )

    # Combinar os gráficos
    chart = alt.layer(line_chart, vertical_lines).resolve_scale(
        color='independent' # Permite que a legenda de cores das linhas e das transições seja separada
    ).properties(
